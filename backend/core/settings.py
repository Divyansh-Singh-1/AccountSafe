...
DEFAULT_FROM_EMAIL = os.getenv("DEFAULT_FROM_EMAIL", "AccountSafe <no-reply@accountsafe.local>")
...
vercel_team = os.getenv("VERCEL_TEAM_SLUG", "")
if vercel_team:
    CORS_ALLOWED_ORIGIN_REGEXES.append(fr"^https://.*-{vercel_team}\.vercel\.app$")
else:
    # Fallback to original owner's team slug if not custom configured
    CORS_ALLOWED_ORIGIN_REGEXES.append(r"^https://.*-pankaj-binds-projects\.vercel\.app$")
if DEBUG:
    CORS_ALLOWED_ORIGIN_REGEXES += [
