...
import { DotGrid, HyperText, MagicCard } from '../components/ui';
...
const LoginRecordRow: React.FC<{ record: LoginRecord; isLast: boolean; index: number }> = ({ record, isLast, index }) => {
  ...
  return (
    <motion.tr
      ...
      className="group border-b border-slate-100 dark:border-slate-800/60 last:border-0 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors"
    >
      <td className="sticky left-0 z-10 bg-white dark:bg-slate-900/90 py-3.5 px-4">
        <div className="flex items-center gap-2">
          {isDuress ? (
            <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 whitespace-nowrap">
              <svg className="w-3.5 h-3.5 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
              </svg>
              <span className="text-[10px] font-bold uppercase tracking-wider text-amber-500">Duress</span>
            </div>
          ) : isSuccess ? (
            <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 whitespace-nowrap">
              <CheckCircleIcon className="w-3.5 h-3.5 text-emerald-500" />
              <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-500">Success</span>
            </div>
          ) : (
            <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-rose-500/10 border border-rose-500/20 whitespace-nowrap">
      ...
      <td className="py-3.5 px-4 whitespace-nowrap">
        ...
        <div className="text-xs text-slate-400 dark:text-slate-500">{dateTime.timeOnly}</div>
      </td>
      <td className="py-3.5 px-4 whitespace-nowrap">
        <code className={`text-xs font-mono px-2 py-0.5 rounded border ${
          ip.isUnknown 
            ? 'text-slate-400 dark:text-slate-500 bg-slate-50/50 dark:bg-slate-950/20 border-slate-100 dark:border-slate-900 italic'
            : 'text-slate-700 dark:text-slate-300 bg-slate-100/50 dark:bg-slate-950/40 border-slate-200/50 dark:border-slate-800/80'
          }`}>
          {ip.display}
        </code>
      </td>
      <td className="py-3.5 px-4">
        <div className="flex items-center gap-1.5">
          {country.isUnknown ? (
            <svg className="w-3.5 h-3.5 text-slate-300 dark:text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9 5.25h.008v.008H12v-.008z" />
            </svg>
          ) : (
            <GlobeIcon className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500" />
          )}
      ...
      <td className="py-3.5 px-4">
        {record.latitude != null && record.longitude != null ? (
          ...
          <span className="text-xs text-slate-400 dark:text-slate-600 italic">Not available</span>
        )}
      </td>
    </motion.tr>
  );
...
const DashboardPage: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 dark:bg-[#09090b] transition-colors duration-300">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <header className="mb-6 sm:mb-8">
          <HyperText text="Dashboard" className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white" />
          <motion.p 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-sm sm:text-base text-slate-500 dark:text-slate-400 mt-1.5"
          >
            Welcome back, <span className="text-slate-800 dark:text-slate-200 font-semibold">{displayName}</span>
          </motion.p>
        </header>
        ...
        {/* ═══════════════════════════════════════════════════════════════════════
            Login Records Table wrapped inside Spotlight MagicCard
            ═══════════════════════════════════════════════════════════════════════ */}
        <section>
          <MagicCard className="bg-white/70 dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/80 dark:border-slate-800/80 rounded-2xl shadow-xl">
            {/* Table Header */}
            <div className="flex items-center justify-between px-6 py-4.5 border-b border-slate-100 dark:border-slate-850">
              <div>
                <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">Login Activity</h2>
                <p className="text-xs sm:text-sm text-slate-400 dark:text-slate-500 font-medium">Monitor your recent login attempts and locations</p>
              </div>
              {!showAllRecords && stats?.recent_logins && stats.recent_logins.length > 0 && (
                <button
                  onClick={fetchAllLoginRecords}
                  className="text-xs sm:text-sm text-indigo-500 hover:text-indigo-650 font-bold transition-colors"
                >
                  View All
                </button>
              )}
            ...
            {displayRecords.length === 0 ? (
              <div className="py-12 sm:py-16 text-center">
                <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-4 rounded-xl bg-slate-100 dark:bg-slate-850 flex items-center justify-center">
                  <ActivityIcon className="w-6 h-6 sm:w-8 sm:h-8 text-slate-400 dark:text-slate-500" />
                </div>
                <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white mb-2">No login records yet</h3>
                ...
                <div className="inline-block min-w-full align-middle">
                  <div className="overflow-hidden">
                    <table className="min-w-full">
                      <thead>
                        <tr className="bg-slate-50/50 dark:bg-slate-950/20 text-xs text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-850 font-semibold">
                          <th className="sticky left-0 z-10 bg-slate-50/50 dark:bg-slate-900/90 text-left py-3.5 px-4 whitespace-nowrap">Status</th>
                          <th className="text-left py-3.5 px-4 whitespace-nowrap">Date & Time</th>
                          <th className="text-left py-3.5 px-4 whitespace-nowrap">IP Address</th>
                          <th className="text-left py-3.5 px-4 whitespace-nowrap">Location</th>
                          <th className="text-left py-3.5 px-4 whitespace-nowrap">ISP</th>
                          <th className="text-left py-3.5 px-4 whitespace-nowrap">Coordinates</th>
                        </tr>
                      </thead>
