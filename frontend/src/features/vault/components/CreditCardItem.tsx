...
const CreditCardItem: React.FC<CreditCardItemProps> = ({
  ...
  return (
    ...
    >
      {/* Credit Card Visual */}
      <div className="transition-all duration-300 group-hover:scale-[1.02] relative rounded-2xl overflow-hidden shadow-lg">
        {profile.is_pinned && <BorderBeam duration={6} size={180} />}
        <CreditCard
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${
              copiedField === `cardnum-${profile.id}`
                ? 'bg-green-500/20 text-green-400 border-green-500/30 shadow-[0_0_12px_rgba(34,197,94,0.1)]'
                : 'bg-zinc-100 dark:bg-zinc-900 text-zinc-650 dark:text-zinc-400 border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700'
            }`}
          >
            {copiedField === `cardnum-${profile.id}` ? (
              <motion.span
