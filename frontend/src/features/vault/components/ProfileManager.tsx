...
import { HyperText, DotGrid } from '../../../components/ui';
...
const ProfileManager: React.FC<ProfileManagerProps> = ({ organization, onBack }) => {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  return (
    <div className="relative min-h-screen bg-slate-50 dark:bg-[#09090b] transition-colors duration-300 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 sm:py-6">
        {/* Back Button */}
        <button
          onClick={onBack}
          ...
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              {orgData?.name ? (
                <HyperText
                  text={orgData.name}
                  className="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white truncate"
                />
              ) : (
                <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white truncate animate-pulse">
                  Loading...
                </h1>
              )}
              <span className="text-slate-400 dark:text-slate-500 text-xs sm:text-sm mt-0.5 block">
...
const LoadingSpinner: React.FC = () => (
  <div className="flex flex-col items-center justify-center py-20 px-4">
    <style>{`
      ...
      @keyframes bolt-retract-top {
        0%, 100% { transform: translateY(0); }
        50% { transform: translateY(8px); }
      }
      @keyframes bolt-retract-bottom {
        0%, 100% { transform: translateY(0); }
        50% { transform: translateY(-8px); }
      }
      @keyframes bolt-retract-left {
        0%, 100% { transform: translateX(0); }
        50% { transform: translateX(8px); }
      }
      @keyframes bolt-retract-right {
        0%, 100% { transform: translateX(0); }
        50% { transform: translateX(-8px); }
      }
    ...
    <div className="relative w-36 h-36 flex items-center justify-center animate-[vault-pulse-glow_2s_infinite_ease-in-out]">
      {/* Glow behind vault */}
      <div className="absolute inset-0 bg-blue-500/10 dark:bg-blue-500/5 rounded-full blur-2xl" />
      ...
      >
        {/* Outer Ring / Vault Frame */}
        <circle cx="60" cy="60" r="54" fill="none" stroke="currentColor" strokeWidth="4" className="text-slate-300 dark:text-slate-800" />
        <circle cx="60" cy="60" r="50" fill="none" stroke="currentColor" strokeWidth="1" strokeDasharray="4 4" className="text-slate-450 dark:text-slate-650" />
        
        {/* Vault Locking Bolts */}
        <rect x="56" y="2" width="8" height="12" rx="2" fill="currentColor" className="text-blue-500 origin-center" style={{ animation: 'bolt-retract-top 3s infinite ease-in-out' }} />
        <rect x="56" y="106" width="8" height="12" rx="2" fill="currentColor" className="text-blue-500 origin-center" style={{ animation: 'bolt-retract-bottom 3s infinite ease-in-out' }} />
        <rect x="2" y="56" width="12" height="8" rx="2" fill="currentColor" className="text-blue-500 origin-center" style={{ animation: 'bolt-retract-left 3s infinite ease-in-out' }} />
        <rect x="106" y="56" width="12" height="8" rx="2" fill="currentColor" className="text-blue-500 origin-center" style={{ animation: 'bolt-retract-right 3s infinite ease-in-out' }} />

        {/* Heavy Outer Rotating Gear */}
        <g style={{ animation: 'vault-spin-outer 12s infinite linear', transformOrigin: '60px 60px' }}>
          <circle cx="60" cy="60" r="40" fill="none" stroke="currentColor" strokeWidth="3" className="text-slate-400 dark:text-slate-700" />
        ...
        {/* Inner Dial / Mechanical Spin */}
        <g style={{ animation: 'vault-spin-inner 6s infinite linear', transformOrigin: '60px 60px' }}>
          <circle cx="60" cy="60" r="28" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-blue-500/80 dark:text-blue-500/50" />
          <circle cx="60" cy="60" r="24" fill="none" stroke="currentColor" strokeWidth="1" strokeDasharray="3 6" className="text-indigo-400" />
          {[0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330].map((angle, i) => {
            const rad = (angle * Math.PI) / 180;
            const x1 = 60 + Math.cos(rad) * 24;
            const y1 = 60 + Math.sin(rad) * 24;
        ...
        <circle cx="60" cy="60" r="16" fill="currentColor" className="text-slate-200 dark:text-slate-900 border border-slate-300 dark:border-slate-800" />
        <circle cx="60" cy="60" r="12" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-slate-400 dark:text-slate-700" />
        
        {/* Pulsing Core Security LED Indicator */}
        <circle cx="60" cy="60" r="5" fill="currentColor" className="text-emerald-500" />
      </svg>
    </div>

    <div className="mt-8 text-center space-y-2">
      <h3 className="text-base font-bold text-slate-800 dark:text-slate-200 uppercase tracking-widest animate-[decrypt-text_2s_infinite]">
        Decrypting Secure Vault
      </h3>
      <p className="text-xs text-slate-500 dark:text-slate-500 font-mono tracking-wider">
        Reassembling zero-knowledge envelopes...
      </p>
    </div>
  </div>
);
