...
import { DotGrid, MagicCard, Input, SearchInput, Textarea, ShimmerButton } from '../../../components/ui';
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8">
        ...
        {!isLoading && categories.length === 0 && (
          <motion.div 
            ...
            className="as-card p-6 sm:p-8 md:p-12 select-none"
          >
            <EmptyState
...
const StatsBar: React.FC<StatsBarProps> = ({ stats }) => (
  <div className="grid grid-cols-3 gap-2 sm:gap-3 md:gap-4 mb-4 sm:mb-6 md:mb-8 select-none">
    <StatCard 
      icon={<FolderIcon className="w-4 h-4 sm:w-5 sm:h-5 text-purple-600 dark:text-purple-400" />} 
    ...
    />
    <StatCard 
      icon={<GlobeIcon className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 dark:text-blue-400" />} 
    ...
    />
    <StatCard 
      icon={<KeyIcon className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-600 dark:text-emerald-400" />} 
...
const StatCard: React.FC<{ icon: React.ReactNode; value: number; label: string; color: string; spotlightColor: string }> = ({ icon, value, label, color, spotlightColor }) => (
  <MagicCard 
    spotlightColor={spotlightColor}
    className="p-2 sm:p-3 md:p-4 flex flex-col sm:flex-row items-center gap-2 sm:gap-3 md:gap-4 select-none"
  >
    <div className={`p-2 sm:p-2.5 md:p-3 bg-${color}-500/10 rounded-lg sm:rounded-xl border border-${color}-500/20`}>
      {icon}
    </div>
...
const SearchBar: React.FC<SearchBarProps> = ({ searchQuery, onSearchChange, onNewCategory, showPulse }) => (
  <div className="sticky top-0 z-10 bg-white dark:bg-[#09090b] pb-4 sm:pb-6 md:pb-8 -mx-3 px-3 sm:-mx-4 sm:px-4 md:-mx-6 md:px-6 lg:-mx-8 lg:px-8 pt-2 select-none">
    <div className="flex gap-2 sm:gap-3 items-end">
      <div className="flex-1">
        <SearchInput
          ...
          inputSize="lg"
          className="bg-white/50 dark:bg-zinc-950/40 border-slate-200 dark:border-slate-800/80"
        />
      </div>
      <ShimmerButton
        onClick={onNewCategory}
        className={`flex items-center justify-center gap-2 group h-12 py-3 px-4 whitespace-nowrap ${showPulse ? 'animate-pulse' : ''}`}
      >
...
const Alert: React.FC<{ message: string; onClose: () => void }> = ({ message, onClose }) => (
  <div className="as-alert-danger mb-6 flex items-center justify-between select-none">
...
const NoResults: React.FC<{ searchQuery: string }> = ({ searchQuery }) => (
  <div className="as-card p-6 sm:p-8 md:p-12 text-center select-none">
...
const CategoryModal: React.FC<CategoryModalProps> = ({
  ...
  >
    ...
    >
      {/* Modal Header */}
      <div className="flex items-center justify-between p-4 sm:p-5 md:p-6 border-b border-zinc-300 dark:border-zinc-800 select-none">
      ...
      <form onSubmit={onSubmit} className="p-4 sm:p-5 md:p-6 space-y-4 sm:space-y-5">
        <div>
          <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2 select-none">
          ...
          <Input
            ...
            autoFocus
            required
            className="bg-white/50 dark:bg-slate-950/40 border-slate-200 dark:border-slate-800/80"
          />
        </div>
        ...
        {/* Modal Actions */}
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-2 select-none">
          <button
            type="button"
            onClick={onClose}
          ...
          <ShimmerButton type="submit" className="w-full sm:flex-1 h-11 text-sm sm:text-base flex items-center justify-center">
            Create Category
          </ShimmerButton>
        </div>
      </form>
      ...
      >
        ...
        <form onSubmit={onSubmit} className="p-4 sm:p-5 md:p-6 space-y-4 sm:space-y-5">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-2 select-none">
              {isDigitalWallet 
          ...
          {(newOrg.name || newOrg.logo_url) && (
            <div className={`
              p-4 rounded-xl border-2 flex items-center gap-4 transition-all select-none
